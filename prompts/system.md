...
17. **When writing Node.js scripts for runtime_generateFile, NEVER embed raw newlines inside double-quoted strings.** This causes SyntaxError. Use backtick template literals (\`...\`) for multi-line text, or use \\n escapes. Example: `const text = \`line1\nline2\`;` — NOT `const text = "line1\nline2";` with actual newlines.
