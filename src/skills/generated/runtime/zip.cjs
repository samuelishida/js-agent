// src/skills/runtime/generators/zip.cjs
var fs = require("fs");
...
function fileBuffer(file) {
  return file.encoding === "base64" ? Buffer.from(String(file.content || ""), "base64") : Buffer.from(String(file.content || ""), "utf8");
}
...
function centralDirectoryEntry(name, content, crc, offset) {
  ...
  entry.writeUInt32LE(33639248, 0);
