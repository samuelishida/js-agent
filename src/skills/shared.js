...
// Re-export hub: delegates to extracted skill modules.
// This file preserves the original window.AgentSkills API while
// the actual logic lives in skill-registry.js, skill-planner.js,
// skill-preflight.js, skill-broadcast.js, skill-executor.js, skill-memory.js.
//
// Load order: skill modules must load BEFORE this file.

(() => {
  'use strict';

  const Planner = window.AgentSkillPlanner || {};
  const Preflight = window.AgentSkillPreflight || {};
  ...
  const registryRuntime = typeof registryModuleFactory === 'function'
    ? registryModuleFactory({
        web_search: (args = {}, context = {}) => {
          ...
          return Executor.runSearchSkills(recoveredQuery);
        },
        ...
        snapshot_skill_catalog: args => Registry.snapshotSkillCatalog(args, Executor.formatToolResult)
      })
  ...
  window.AgentSkills = {
    ...
    extractEntities: (window.AgentSkillCore?.intents?.extractEntities || (() => ({ urls: [], currencies: [] }))),
    detectFxPair: (window.AgentSkillCore?.intents?.detectFxPair || (() => null)),
    formatToolResult: Executor.formatToolResult,
    buildPreflightPlan: Preflight.buildPreflightPlan,
    runSearchSkills: Executor.runSearchSkills,
    fetchReadablePage: Executor.fetchReadablePage,
    getToolExecutionMeta: (window.AgentSkillCore?.toolMeta?.getToolExecutionMeta || (() => ({ readOnly: false, concurrencySafe: false, destructive: false, riskLevel: 'elevated' }))),
    canRunToolConcurrently: (window.AgentSkillCore?.toolMeta?.canRunToolConcurrently || (call => !!(window.AgentSkillCore?.toolMeta?.getToolExecutionMeta?.(call?.tool)?.concurrencySafe))),
