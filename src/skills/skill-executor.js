...
// Skill execution: web/data/fs runtime wrappers, worker tasks, compat tools, path utilities.
// Reads from window.AgentSkillModules, window.AgentSkillBroadcast, window.AgentSkillMemory.
// Publishes: window.AgentSkillExecutor
...
(() => {
  ...
  const webRuntime = typeof webModuleFactory === 'function'
    ? webModuleFactory({ formatToolResult, detectFxPair: (window.AgentSkillCore?.intents?.detectFxPair || (() => null)), detectWeatherIntent: (window.AgentSkillCore?.intents?.detectWeatherIntent || (() => false)), detectRecencyIntent: (window.AgentSkillCore?.intents?.detectRecencyIntent || (() => false)), detectCodingIntent: (window.AgentSkillCore?.intents?.detectCodingIntent || (() => false)), detectBiographicalFactIntent: (window.AgentSkillCore?.intents?.detectBiographicalFactIntent || (() => false)) })
    : {};
  ...
  const state = window.AgentSkills?.state || { roots: new Map(), defaultRootId: null, uploads: new Map() };
  ...
  async function runtimeFileDiff({ path, newContent } = {}) {
    try {
      const fsRt = window.AgentSkillModules?.createFilesystemRuntime?.({ state: { roots: new Map(), defaultRootId: null }, formatToolResult, supportsFsAccess: () => !!window.showDirectoryPicker, supportsTextPreview: () => true });
