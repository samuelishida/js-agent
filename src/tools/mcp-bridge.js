...
// Discovers tools from configured MCP servers and registers them into AgentTools.registry.
// Call discoverAndRegisterMcpTools() after AgentTools is initialized.
...
(() => {
  ...
  async function discoverAndRegisterMcpTools() {
    ...
    if (!agentSkills?.registry || !agentSkills?.toolGroups || !agentSkills?.formatToolResult) return;
    ...
    for (const server of servers) {
      ...
      try {
        ...
        if (!toolGroups[groupKey]) {
          toolGroups[groupKey] = { label: `MCP: ${server.name || serverSlug}`, tools: [] };
        }
