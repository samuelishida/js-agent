...
(() => {
  ...
  const compatTools = [
    ...
    { name: 'runtime_generateFile', signature: 'runtime_generateFile(path, content, cwd?, command?)', description: 'Writes a script file and executes it via the local dev server bridge. Use for generating binary files (DOCX, PDF, etc.) server-side and then downloading them. The optional command overrides the default node execution.', run: Executor.runtimeGenerateFile },
