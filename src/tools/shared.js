...
(() => {
  ...
  const compatTools = [
    ...
    { name: 'runtime_generateFile', signature: 'runtime_generateFile(path, content, filename?, cwd?, command?)', description: 'Generate binary files (DOCX, PDF, XLSX, PPTX, PNG). Pass a Node.js generator script ending in .cjs as content; it writes base64 to stdout, auto-saves the final file, and registers an artifact. In packaged Electron, saves to the app Generated Files folder; in browser, downloads to Downloads. Always pass filename="output.ext". Do not use storage_set, runtime_runTerminal, or fs_download_file after this unless auto-save fails.', run: Executor.runtimeGenerateFile },
