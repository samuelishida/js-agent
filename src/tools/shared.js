...
(() => {
  ...
  const compatTools = [
    ...
    { name: 'runtime_generateFile', signature: 'runtime_generateFile(path, content?, cwd?, command?, storageKey?, filename?)', description: 'Generate binary files (DOCX, PDF, XLSX, PPTX, PNG). Writes a Node.js script to the sandbox, executes it, and AUTO-DOWNLOADS the result. The file lands in the user\'s Downloads folder — do NOT call fs_download_file or runtime_runTerminal after this. Script must end with process.stdout.write(base64String). Available packages: docx, pdfkit, exceljs, pptxgenjs, archiver, jszip. Use .cjs extension. Use filename param to set the download name.', run: Executor.runtimeGenerateFile },
