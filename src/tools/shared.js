...
(() => {
  ...
  const compatTools = [
    ...
    { name: 'runtime_generateFile', signature: 'runtime_generateFile(path, content?, cwd?, command?, storageKey?)', description: 'Generate binary files (DOCX, PDF, XLSX, PPTX, PNG). Writes a Node.js script to the sandbox and executes it. AUTO-DOWNLOADS the result — do NOT call fs_download_file after this. Script must end with process.stdout.write(base64String). Available packages: docx, pdfkit, exceljs, pptxgenjs, archiver, jszip. Use .cjs extension. Prefer content param for script source.', run: Executor.runtimeGenerateFile },
