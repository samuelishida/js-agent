(() => {
  ...
  function getBundledTools() {
    const skills = getManifest().bundledTools;
  ...
  function toSnapshotToolName(toolName) {
    const normalized = String(toolName || '')
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '_')
      .replace(/^_+|_+$/g, '');
    return normalized ? `snapshot_tool_${normalized}` : 'snapshot_tool_unknown';
  }

  function searchBundledTools({ query = '', limit = 20 } = {}) {
    const q = normalizeQuery(query);
    const max = Math.max(1, Math.min(100, Number(limit) || 20));
    const all = getBundledTools();
    const filtered = q
      ? all.filter(item => {
          const haystack = [
            item.name,
  ...
  function getPromptAddendum() {
    ...
    const tools = getBundledTools().slice(0, 30);
    if (!tools.length) return '';

    const toolLines = tools.map(item => {
    ...
    const sections = [
      '# Imported Snapshot Tools',
      'The runtime loaded a sanitized tool catalog extracted from the snapshot bundle.',
  ...
  function formatToolCatalogForTool({ query = '', limit = 20 } = {}) {
    const matches = searchBundledTools({ query, limit });
    if (!matches.length) {
      return 'No imported snapshot tools matched the query.';
    }
